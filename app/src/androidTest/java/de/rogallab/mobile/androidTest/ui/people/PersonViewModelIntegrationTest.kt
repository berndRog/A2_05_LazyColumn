...
class PersonViewModelIntegrationTest : KoinTest {
   ...
   private val repository: IPersonRepository by inject()
   ...
   fun fetch_loads_list() = runTest {
      val expected = Seed().people
      // act: MVI Intent
      viewModel.handlePeopleIntent(PeopleIntent.Fetch)
      // assert: actual state
      val actualPeopleUiState = viewModel.peopleUiStateFlow.value
   ...
   fun fetchById_loads_person() = runTest {
      ...
      viewModel.handlePeopleIntent(PeopleIntent.Fetch)
      val expected = viewModel.peopleUiStateFlow.value.people.first()
      val id = expected.id
      ...
      // assert: actual state
      val actualPersonUiState = viewModel.personUiStateFlow.value
   ...
   fun create_increases_list() = runTest {
      ...
      viewModel.handlePeopleIntent(PeopleIntent.Fetch)
      val initialPeopleUiState = viewModel.peopleUiStateFlow.value
      ...
      viewModel.handlePersonIntent(PersonIntent.FirstNameChange(expectedFirstName))
      viewModel.handlePersonIntent(PersonIntent.LastNameChange(expectedLastName))
      viewModel.handlePersonIntent(PersonIntent.Create)   // ruft intern fetch()
   ...
   fun update_changes_list() = runTest {
      ...
      viewModel.handlePersonIntent(PersonIntent.FetchById(id))
      viewModel.handlePersonIntent(PersonIntent.LastNameChange(updatedLastName))
      viewModel.handlePersonIntent(PersonIntent.Update)

      // assert: people list contains the changed item
      val peopleUiState = viewModel.peopleUiStateFlow.value
   ...
   fun remove_reduces_list() = runTest {
      ...
      viewModel.handlePersonIntent(PersonIntent.Remove(victim))
